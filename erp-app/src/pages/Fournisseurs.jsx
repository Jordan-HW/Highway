...
export default function Fournisseurs() {
  ...
  async function save() {
    ...
    let error
    if (editing) {
      const { error: e } = await supabase.from('fournisseurs').update(form).eq('id', editing)
      error = e
    } else {
      const { error: e } = await supabase.from('fournisseurs').insert(form)
      error = e
    }
    setSaving(false)
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          <h2>Fournisseurs</h2>
          <p>{rows.length} fournisseur{rows.length > 1 ? 's' : ''}</p>
        </div>
                  ...
                  {filtered.length === 0 ? (
                    <tr><td colSpan={7}>
                      <div className="empty-state">
                        <Building2 />
                        <p>Aucun fournisseur. Créez-en un !</p>
                      </div>
                    </td></tr>
                  ) : filtered.map(row => (
            ...
            <div className="modal-body">
              ...
              <div className="form-grid">
                <div className="form-group form-full">
                  <label>Nom *</label>
                  <input value={form.nom} onChange={e => set('nom', e.target.value)} placeholder="Nom du fournisseur" />
                </div>
