...
export default function Tarifs() {
  ...
  const margeHandled = useRef(false)
                          ...
                          <tr key={p.id} style={{ background: dirty ? '#FFFDE7' : isExpanded ? 'var(--primary-light)' : undefined }}>
                            ...
                            <td style={{ padding: '3px 4px', verticalAlign: 'middle' }}>
                              {editingMarge === `hw-${p.id}` ? (
                                <input type="number" step="0.1" autoFocus defaultValue={marge != null ? marge.toFixed(2) : ''}
                                  onFocus={() => { margeHandled.current = false }}
                                  onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); margeHandled.current = true; const v = parseFloat(e.target.value); if (!isNaN(v)) handleMargeHWChange(p.id, e.target.value); setEditingMarge(null) } if (e.key === 'Escape') { margeHandled.current = true; setEditingMarge(null) } }}
                                  onBlur={e => { if (margeHandled.current) return; const v = parseFloat(e.target.value); if (!isNaN(v)) handleMargeHWChange(p.id, e.target.value); setEditingMarge(null) }}
                            ...
                            <td style={{ padding: '3px 4px', verticalAlign: 'middle' }}>
                              {editingMarge === `cl-${p.id}` ? (
                                <input type="number" step="0.1" autoFocus defaultValue={margeClient != null ? margeClient.toFixed(2) : ''}
                                  onFocus={() => { margeHandled.current = false }}
                                  onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); margeHandled.current = true; handleMargeClientBlur(p.id, e.target.value) } if (e.key === 'Escape') { margeHandled.current = true; setEditingMarge(null) } }}
                                  onBlur={e => { if (margeHandled.current) return; const v = parseFloat(e.target.value); if (!isNaN(v)) handleMargeClientBlur(p.id, e.target.value); else setEditingMarge(null) }}
                                  style={{ padding: '3px 4px', fontSize: 11, width: '100%', textAlign: 'right' }} />
